import { validateBirthDate } from "./date";
...
export function validateRegisterForm(formData: {
  ...
  if (!formData.password || !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{6,}$/.test(formData.password)) {
    return { 
      isValid: false, 
      errorMessage: "Пароль должен содержать 6+ символов, включая заглавные, строчные буквы и цифры" 
    };
  }
  ...
  if (!formData.hasCard && formData.card && !/^\d{16}$/.test(formData.card.replace(/\s/g, ""))) {
    return { 
      isValid: false, 

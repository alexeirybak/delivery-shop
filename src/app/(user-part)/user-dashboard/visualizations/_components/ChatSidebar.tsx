...
export const ChatSidebar = ({
  ...
  const loadChats = async () => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/visualizations/list");
      if (response.ok) {
  ...
  const searchChats = useCallback(async (query: string) => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch("/api/visualizations/search", {
        method: "POST",
  ...
  const getModeLabel = (mode: string): string => {
    const labels: Record<string, string> = {
      ...
      network: "Сетевой граф",
      barchart: "Столбчатая диаграмма",
      piechart: "Круговая диаграмма",
      linechart: "Линейный график",
      radarchart: "Лепестковая диаграмма",
      visualization_comparison: "Сравнительная таблица",
      roadmap: "Дорожная карта",
      hierarchy: "Иерархическая схема",
      timeline: "Хронология",
      flashcards: "Карточки",
      glossary: "Глоссарий",

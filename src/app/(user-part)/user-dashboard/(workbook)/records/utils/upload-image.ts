...
export const uploadToServer = async (file: File): Promise<UploadResult> => {
  ...
  const response = await fetch("/api/workbook/records/upload/temp-image", {
    method: "POST",
    body: formData,
  });
...
export const insertImageToEditor = (
  ...
  const cleanAlt = cleanFileName(alt);
  const cleanTitle = title ? cleanFileName(title) : cleanAlt;

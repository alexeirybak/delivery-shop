...
import { UpdateCategoryData } from "../../records/types/categories/categories.types";
...
export const useCategories = () => {
  ...
  const createCategory = async (
    ...
    try {
      const response = await fetch("/api/workbook/categories", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        return {
          success: true,
          message: data.message || "Тетрадь успешно создана",
        };
      } else {
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Ошибка сети при создании тетради",
      };
    }
  };
  ...
  const deleteCategory = async (id: string): Promise<ApiResponse> => {
    try {
      const response = await fetch(`/api/workbook/categories/${id}`, {
        method: "DELETE",
      });
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Ошибка сети при удалении тетради",
      };
    }
  };
    ...
    try {
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Ошибка сети при обновлении тетради",
      };
    }
  };
  ...
  const reorderCategories = async (
    ...
    try {
      const response = await fetch("/api/workbook/categories/reorder", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },

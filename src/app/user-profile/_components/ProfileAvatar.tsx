...
interface AvatarUploadProps {
  avatar: string;
  gender: string; // оставляем обязательным
  isEditing: boolean;
...
const ProfileAvatar = ({
  ...
  const getCurrentAvatar = () => {
    return avatar || getAvatarPath({ gender }); // передаем объект с gender
  };

  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    const target = e.target as HTMLImageElement;
    target.src = getAvatarPath({ gender }); // используем утилиту для fallback
  };

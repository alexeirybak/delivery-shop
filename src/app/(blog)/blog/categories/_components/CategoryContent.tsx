    ...
    <div className="p-5 md:p-6 flex-1 flex flex-col">
      ...
      <div className="mt-auto pt-4 border-t border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <Link
            href={`/blog/${category}`}
            className="inline-flex items-center justify-center gap-2 rounded px-5 py-2.5 text-sm font-semibold text-white bg-linear-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 transition-custom hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 group/btn"
            aria-label={`Перейти к категории ${name}`}
          >
            <span>Подробнее</span>
            <ArrowRight
              className="w-4 h-4 transition-transform transition-custom group-hover/btn:translate-x-1"
              strokeWidth={2}
            />
          </Link>
        </div>

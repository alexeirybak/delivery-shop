...
import { FakePaymentModalProps, FakePaymentData, PaymentSimulationResult } from '@/types/payment';
...
const FakePaymentModal = ({ 
  ...
  const testCards: TestCard[] = [
    { number: '5555 5555 5555 4444', description: 'Успешная оплата', result: 'success' },
    { number: '4111 1111 1111 1111', description: 'Недостаточно средств', result: 'failure' },
    { number: '4000 0000 0000 0002', description: 'Ошибка банка', result: 'error' },
  ];

  const simulatePayment = async (simulatedResult: PaymentSimulationResult): Promise<void> => {
    if (!isOpen) return;
  ...
  const fillTestCard = (cardNumber: string, result: PaymentSimulationResult): void => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        ...
        <div className="mb-4 p-3 bg-gray-50 rounded">
          <h3 className="text-sm font-medium mb-2">Тестовые карты (авто-оплата):</h3>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Имя держателя</label>
            <input
              type="text"
              value={cardholder}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={isProcessing || !cardNumber || !expiryDate || !cvc || !cardholder}
            ...
            >
              {isProcessing ? 'Обработка...' : `Оплатить ${formatPrice(amount)} ₽`}
            </button>
          </div>
        </form>
      </div>

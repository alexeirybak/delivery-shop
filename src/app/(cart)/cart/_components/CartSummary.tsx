...
import { prepareCartItemsWithPrices, createOrderRequest, updateUserAfterPayment } from "../utils/orderHelpers";
...
const CartSummary = ({ deliveryData, productsData = {} }: CartSummaryProps) => {
  ...
  const createOrder = async (paymentMethod: "cash_on_delivery" | "online", paymentId?: string) => {
    if (!deliveryData) {
    ...
    try {
      ...
      if (paymentMethod === "online") {
        try {
          ...
          console.warn("Заказ создан, но возникла проблема с обновлением бонусов", updateError);
        }

...
async function getYandexKeys(headers: Headers): Promise<YandexKeys | null> {
  try {
    ...
    if (user?.yandexApiKey && user?.yandexFolderId) {
      try {
        ...
        console.error("[OCR-Handwritten] Ошибка расшифровки:", error);
      }
    }
  } catch (error) {
    console.error(
      "[OCR-Handwritten] Ошибка получения пользовательских ключей:",
      error,
    );
  }
  ...
  console.error("[OCR-Handwritten] Нет доступных API ключей!");
  return null;
}
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          details: "Нет доступных API ключей.",
        },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      {
        error: "Ошибка распознавания рукописного текста",

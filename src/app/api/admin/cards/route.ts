...
import { FilterType } from "@/app/(admin)/administrator/cards/types/cards.types";
import { getDB } from "../../../../../utils/api-routes";
import { CONFIG_CARDS } from "@/app/(admin)/administrator/cards/utils/CONFIG_CRADS";
...
export async function PATCH(request: Request) {
  try {
    ...
    return NextResponse.json({ message: "Неверное действие" }, { status: 400 });
  } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (userWithCard) {
      await db
        .collection("user")
        .updateOne({ _id: userWithCard._id }, { $unset: { card: "" } });
    }

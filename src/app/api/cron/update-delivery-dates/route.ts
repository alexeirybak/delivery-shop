...
export async function GET(request: NextRequest) {
  try {
    ...
    datesToAdd.forEach(newDate => {
      const sampleDate = newDates.find(date => updatedSchedule[date]) || currentDates[0];
      if (sampleDate && updatedSchedule[sampleDate]) {
        updatedSchedule[newDate] = { ...updatedSchedule[sampleDate] };
      } else {
